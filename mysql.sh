...
function mysql::connect {
  ...
  local db=$(grep database "${defaults_file}"|cut -d'=' -f2)
  if [[ -z ${db} ]]; then
    if [[ -n $2 ]]; then
      local -r db=$2
    else
      log::die "Cannot connect mysql, database missing in defaults file and not specified as argument" "file" "${defaults_file}"
    fi
  fi
...
function mysql::sql {
  ...
  while true; do
    ...
    if [[ -n ${errno} ]]; then
      log::error "mysql error" errno ${errno} db ${db} status ${status}
      return ${errno};
    fi
